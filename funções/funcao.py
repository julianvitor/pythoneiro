...
def terceira_funcao(
    *cidades,
):  # usando (*parametro) é possivle passar uma tupla como argumento
...
def quarta_funcao(
    nome1, nome2, nome3
):  # é possivel especificar os parametros no argumento independende da ordem utilizando chave-valor
...
def quinta_funcao(
    **valor,
):  # atribuição através de chave-valor sem esperar um numero de argumentos (def x(*y))
...
def sexta_funcao(
    comida="pão",
):  # valor padrão ja interno na declaração da funcao para caso de não expecificação
...
def setima_funcao(
    arte,
):  # os tipos se mantem passando em uma função, (listas continuam sendo lista)
